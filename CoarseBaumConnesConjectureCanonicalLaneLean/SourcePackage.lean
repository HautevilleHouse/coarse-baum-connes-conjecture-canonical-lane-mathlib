/-!
# Source package model for `coarse-baum-connes-conjecture-canonical-lane`
...
def sourceCheckoutHead : String := "14cc8335d449789f65245c71dd3cbcbb64c78eef"
def sourceCheckoutClean : Bool := true

def sourceFiles : List SourceFile := [
  { path := "scripts/cbc_closure_guard.py", sha256 := "b2451e00ff0640077bce1128e6d80289a12b3b06a13d9dd249535e8268bf3834", functionCount := 9, classCount := 0, assignmentCount := 8, parseOk := true },
...
def sourceFunctions : List SourceFunctionDecl := [
  { file := "scripts/cbc_closure_guard.py", name := "_finite", args := ["v"], returns := "bool", doc := "", line := 31, isAsync := false },
  { file := "scripts/cbc_closure_guard.py", name := "_resolve", args := ["path_str"], returns := "Path", doc := "", line := 35, isAsync := false },
  { file := "scripts/cbc_closure_guard.py", name := "_bootstrap_registry", args := ["path"], returns := "None", doc := "", line := 42, isAsync := false },
  { file := "scripts/cbc_closure_guard.py", name := "_load_registry", args := ["path"], returns := "dict[str, Any]", doc := "", line := 48, isAsync := false },
  { file := "scripts/cbc_closure_guard.py", name := "_load_sigma_from_stitch", args := ["path"], returns := "float | None", doc := "", line := 60, isAsync := false },
  { file := "scripts/cbc_closure_guard.py", name := "_entry", args := ["constants", "key"], returns := "tuple[float | None, bool]", doc := "", line := 77, isAsync := false },
  { file := "scripts/cbc_closure_guard.py", name := "compute_report", args := ["data", "sigma_star", "strict_coh_zero"], returns := "dict[str, Any]", doc := "", line := 88, isAsync := false },
  { file := "scripts/cbc_closure_guard.py", name := "append_history", args := ["path", "report"], returns := "None", doc := "", line := 121, isAsync := false },
  { file := "scripts/cbc_closure_guard.py", name := "main", args := [], returns := "None", doc := "", line := 128, isAsync := false },
...
def sourceAssignments : List SourceAssignmentDecl := [
  { file := "scripts/cbc_closure_guard.py", name := "SCRIPT_DIR", value := "Path(__file__).resolve().parent", line := 13 },
  { file := "scripts/cbc_closure_guard.py", name := "PROJECT_ROOT", value := "SCRIPT_DIR.parent", line := 14 },
  { file := "scripts/cbc_closure_guard.py", name := "DEFAULT_REGISTRY", value := "\"artifacts/constants_registry.json\"", line := 16 },
  { file := "scripts/cbc_closure_guard.py", name := "DEFAULT_STITCH", value := "\"artifacts/stitch_constants.json\"", line := 17 },
  { file := "scripts/cbc_closure_guard.py", name := "DEFAULT_OUT", value := "\"repro/certificate_runtime.json\"", line := 18 },
  { file := "scripts/cbc_closure_guard.py", name := "DEFAULT_HISTORY", value := "\"repro/drift_guard_runs.jsonl\"", line := 19 },
  { file := "scripts/cbc_closure_guard.py", name := "PRIMARY_KEYS", value := "[\"kappa_coarse\", \"sigma_metric\", \"kappa_compact\", \"rho_rigidity\", \"coarse_transfer\"]", line := 21 },
  { file := "scripts/cbc_closure_guard.py", name := "COHERENCE_KEY", value := "\"eps_coh\"", line := 28 },
...
def constantSpecs : List ConstantSpec := [
  { group := "constants", key := "kappa_coarse", status := "derived_numeric", formula := "c_coarse_raw * coarse_density_raw - e_coarse_raw", sourceSection := "paper/COARSE_BAUM_CONNES_CONJECTURE_PREPRINT.md Section 4.5 / Appendix A", validation := "required_positive", componentKeys := ["c_coarse_raw", "coarse_density_raw", "e_coarse_raw"] },
  { group := "constants", key := "sigma_metric", status := "derived_numeric", formula := "metric_floor_raw - coarse_loss_raw - restart_loss_raw", sourceSection := "paper/COARSE_BAUM_CONNES_CONJECTURE_PREPRINT.md Section 5.2 / Appendix B", validation := "required_positive", componentKeys := ["coarse_loss_raw", "metric_floor_raw", "restart_loss_raw"] },
  { group := "constants", key := "kappa_compact", status := "derived_numeric", formula := "1.0 / (1.0 + delta_comp_sup_raw)", sourceSection := "paper/COARSE_BAUM_CONNES_CONJECTURE_PREPRINT.md Section 5.3 / Appendix C", validation := "required_positive", componentKeys := ["delta_comp_sup_raw"] },
  { group := "constants", key := "rho_rigidity", status := "derived_numeric", formula := "rho_rigidity_raw", sourceSection := "paper/COARSE_BAUM_CONNES_CONJECTURE_PREPRINT.md Section 6.1 / Appendix D", validation := "required_positive", componentKeys := ["rho_rigidity_raw"] },
  { group := "constants", key := "coarse_transfer", status := "derived_numeric", formula := "c_transfer_raw * transfer_gain_raw - e_transfer_raw", sourceSection := "paper/COARSE_BAUM_CONNES_CONJECTURE_PREPRINT.md Section 6.2 / Appendix E.4", validation := "required_positive", componentKeys := ["c_transfer_raw", "e_transfer_raw", "transfer_gain_raw"] },
  { group := "constants", key := "eps_coh", status := "derived_numeric", formula := "eps_coh_raw", sourceSection := "paper/COARSE_BAUM_CONNES_CONJECTURE_PREPRINT.md Appendix E.6", validation := "required_nonnegative", componentKeys := ["eps_coh_raw"] },
  { group := "stitch", key := "sigma_star_can", status := "derived_numeric", formula := "sigma_star_can_raw", sourceSection := "paper/COARSE_BAUM_CONNES_CONJECTURE_PREPRINT.md Section 5 / stitch constants", validation := "required_positive", componentKeys := ["sigma_star_can_raw"] }
]

def registryConstants : List RegistryConstant := [
  { key := "coarse_transfer", value := "1.029422", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#coarse_transfer", sourceSection := "paper/COARSE_BAUM_CONNES_CONJECTURE_PREPRINT.md Section 6.2 / Appendix E.4" },
  { key := "eps_coh", value := "0.0", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#eps_coh", sourceSection := "paper/COARSE_BAUM_CONNES_CONJECTURE_PREPRINT.md Appendix E.6" },
  { key := "kappa_coarse", value := "1.0913680000000001", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_coarse", sourceSection := "paper/COARSE_BAUM_CONNES_CONJECTURE_PREPRINT.md Section 4.5 / Appendix A" },
  { key := "kappa_compact", value := "0.8045052292839904", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#kappa_compact", sourceSection := "paper/COARSE_BAUM_CONNES_CONJECTURE_PREPRINT.md Section 5.3 / Appendix C" },
  { key := "rho_rigidity", value := "1.077", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#rho_rigidity", sourceSection := "paper/COARSE_BAUM_CONNES_CONJECTURE_PREPRINT.md Section 6.1 / Appendix D" },
  { key := "sigma_metric", value := "1.073", theoremLevel := true, status := "derived_numeric", source := "artifacts/constants_extracted.json#sigma_metric", sourceSection := "paper/COARSE_BAUM_CONNES_CONJECTURE_PREPRINT.md Section 5.2 / Appendix B" }
]
